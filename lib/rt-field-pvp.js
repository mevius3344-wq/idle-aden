...
/**
 * 野外 PK（同圖、攻擊者開啟 pvpOn）：伺服器權威扣血。
 * v3.9.36：擊殺後被害方回報實際遺失快照 → 原樣轉交擊殺者（不少不多）。
 * 未開 PK 者可被打，但不能還手。
 */
...
const LOOT_TRANSFER_WINDOW_MS = 60 * 1000;
...
function sanitizeLootItem(it) {
  ...
  if (!id) return null;
  let attr = false;
  if (it.attr != null && it.attr !== false && it.attr !== "") {
    attr = String(it.attr).replace(/[<>&"']/g, "").trim().slice(0, 24) || false;
  }
  return {
    id,
    en: Math.max(0, Math.min(15, Math.floor(Number(it.en) || 0))),
    bless: !!it.bless,
    anc: !!it.anc,
    attr,
    seteff: !!it.seteff,
...
function requireAuth(data, res) {
  if (!_antiCheat) return { ok: true };
...
async function handleFieldPvpApi(req, res, u) {
  ...
  if (u === "/api/pvp/hit" && req.method === "POST") {
    ...
    if (!requireAuth(data, res).ok) return;
    ...
    const killed = hp <= 0;
    if (killed) {
      victim.lastPvpKillerKey = fromKey;
      victim.lastPvpKillerName = attacker.name || "冒險者";
      victim.lastPvpKillerAt = now;
      victim.lastPvpLootDone = false;
    }
  ...
  // ⚔️ v3.9.36 被害方回報實際遺失快照 → 原樣推給擊殺者
  if (u === "/api/pvp/loot-transfer" && req.method === "POST") {
    const data = await parseJsonBody(req);
    if (data === null) return jsonRes(res, 400, { ok: false, error: "bad json" });
    if (!requireAuth(data, res).ok) return;
    ...
    const name = partySanitizeName(data.name);
    const fromKey = partyMemberKey(account, slot, name);
    if (!fromKey) return jsonRes(res, 400, { ok: false, error: "bad_identity" });

    const victim = await loadPresence(sql, fromKey);
    if (!victim) return jsonRes(res, 404, { ok: false, error: "offline" });

    const now = Date.now();
    const killerKey = String(victim.lastPvpKillerKey || "").trim().slice(0, 48);
    const killerAt = Number(victim.lastPvpKillerAt) || 0;
    if (!killerKey || now - killerAt > LOOT_TRANSFER_WINDOW_MS) {
      return jsonRes(res, 409, { ok: false, error: "no_kill", message: "沒有可轉交的擊殺紀錄。" });
    }
    if (victim.lastPvpLootDone) {
      return jsonRes(res, 409, { ok: false, error: "already", message: "本場掉落已轉交。" });
    }

    const rawList = Array.isArray(data.items) ? data.items : [];
    const items = [];
    for (let i = 0; i < rawList.length && items.length < MAX_LOOT_ITEMS; i++) {
      const one = sanitizeLootItem(rawList[i]);
      if (one) items.push(one);
    }

    victim.lastPvpLootDone = true;
    await savePresence(sql, victim);

    if (items.length) {
      await pushToKey(sql, killerKey, {
        type: "pvp_loot",
        fromKey,
        fromName: victim.name || name || "對手",
        items,
      });
    }
