...
/** 廣播用角色名：拒收物品 id／控制字元，避免跑馬燈顯示亂碼 */
function sanitizePandoraCharName(raw) {
 ...
 */
async function performDraw(sql, account, slot, qty, charName) {
  ...
  // 珍稀（權重 1）世界廣播：角色名優先雲端存檔；絕不把物品 id 當名稱
  const rares = results.filter((r) => r.weight === 1);
  let who = sanitizePandoraCharName(charName);
  if (!who) {
    try {
      const { accountKey } = require("./game-utils");
      const { normSlot } = require("./rt-cloud-wallet");
      const ak = accountKey(acc);
      const s = normSlot(slot);
      const rows = await sql`SELECT data FROM cloud_slots WHERE account_key = ${ak} AND slot = ${s} LIMIT 1`;
      const nm = rows[0] && rows[0].data && rows[0].data.p && rows[0].data.p.name;
      who = sanitizePandoraCharName(nm);
    } catch (eName) {}
  }
  if (!who) who = "有人";
...
async function handlePandoraApi(req, res, u) {
  ...
  if (u === "/api/pandora/draw" && req.method === "POST") {
    ...
    const result = await performDraw(sql, account, data.slot, data.qty, data.charName);
    if (!result || !result.ok) {
      const code =
