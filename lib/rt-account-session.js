...
const ACCOUNT_IDLE_LOGOUT_MS = Math.max(
  60000,
  Number(process.env.ACCOUNT_IDLE_LOGOUT_MS || 30 * 60 * 1000)
);
...
async function touchAccountSessionSql(sql, accountKey, sessionId, opts) {
  await pruneAccountSessionsSql(sql);
  const now = Date.now();
  const lastActivityMs = Math.min(now, Number(opts && opts.lastActivityMs) || now);
  if (now - lastActivityMs > ACCOUNT_IDLE_LOGOUT_MS) {
    await releaseAccountSessionSql(sql, accountKey, sessionId);
    return {
      ok: false,
      error: "idle_timeout",
      message: "閒置過久，已自動登出。",
...
function createFileAccountSessionStore(filePath) {
  ...
  async function touch(accountKey, sessionId, opts) {
    const now = Date.now();
    const lastActivityMs = Math.min(now, Number(opts && opts.lastActivityMs) || now);
    if (now - lastActivityMs > ACCOUNT_IDLE_LOGOUT_MS) {
      const map = readMap();
  ...
  return { claim, touch, verify, release, countOnline };
}

module.exports = {
  ACCOUNT_SESSION_TTL_MS,
  ACCOUNT_IDLE_LOGOUT_MS,
  newSessionId,
  claimSessionLogic,
