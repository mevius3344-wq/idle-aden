...
async function handleAuctionApi(req, res, u) {
    ...
    try {
      ...
      try {
        await sqlMutateSlot(sql, account, slot, (pdata) => {
          ...
          if (!pdata.p.inv) pdata.p.inv = [];
          pdata.p.inv.push(
            Object.assign({}, item, {
              uid: sourceUid,
              cnt: Math.max(1, Math.floor(Number(item.cnt) || 1)),
            })
          );
          return { ok: true };
        });
  ...
  if (u === "/api/auction/buy" && req.method === "POST") {
    ...
    const wallet = await sqlDebitGold(sql, account, slot, fees.totalBuy);
    if (!wallet.ok) {
      await saveListing(sql, L);
  ...
  if (u === "/api/auction/claim" && req.method === "POST") {
    ...
    return jsonRes(res, 200, {
      ok: true,
      claims: taken,
      message: taken.length ? "領取成功。" : "沒有待領取內容。",
    });
  }
