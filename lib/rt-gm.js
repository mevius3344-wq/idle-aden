...
async function gmSearch(res, sql, q) {
  ...
  const accounts = await sql`
    ...
    WHERE account ILIKE ${likeAny} ORDER BY account LIMIT 30`;
  const chars = await sql`
    SELECT name, account, slot FROM char_names
    WHERE name ILIKE ${likeAny} ORDER BY name LIMIT 30`;
  return jsonRes(res, 200, {
    ok: true,
...
/** GM：目前真正在線的玩家（組隊 presence 優先，帳號 session 補齊） */
async function gmOnlinePlayers(res, sql, q) {
  ...
  const presenceCutoff = now - PARTY_TTL_MS;
  const sessionCutoff = now - ACCOUNT_SESSION_TTL_MS;

  const byAcc = new Map();
  function ensure(acc) {
    const key = String(acc || "").trim();
    if (!key) return null;
  ...
  try {
    const presenceRows = await sql`
      SELECT member_key, payload, last_seen FROM rt_party_presence
      WHERE last_seen >= ${presenceCutoff}
    ...
    for (const r of presenceRows) {
      ...
      if (!row) continue;
      const seen = Number(r.last_seen) || 0;
      if (seen > row.lastSeenMs) row.lastSeenMs = seen;
      const name = String(p.name || "").trim();
      const slot = Number(p.slot) || 0;
      if (name) {
        const exists = row.chars.some((c) => c.name === name && Number(c.slot) === slot);
        if (!exists) {
          row.chars.push({
            slot,
            name,
            lv: Number(p.lv) || 0,
            clsName: String(p.cls || ""),
            mapId: String(p.mapId || ""),
            mapName: String(p.mapName || ""),
          });
        }
      }
      const mapLabel = String(p.mapName || p.mapId || "").trim();
      if (mapLabel && row.maps.indexOf(mapLabel) < 0) row.maps.push(mapLabel);
    }
  } catch (e) {}

  try {
    await sql`DELETE FROM account_sessions WHERE last_seen_ms < ${sessionCutoff}`;
    const sessRows = await sql`
      SELECT s.account_key, s.last_seen_ms, a.account, a.banned_until
      FROM account_sessions s
      LEFT JOIN accounts a ON a.account_key = s.account_key
      WHERE s.last_seen_ms >= ${sessionCutoff}
      ORDER BY s.last_seen_ms DESC LIMIT 200`;
    for (const r of sessRows) {
      const acc = String(r.account || r.account_key || "").trim();
      const row = ensure(acc);
      if (!row) continue;
      const seen = Number(r.last_seen_ms) || 0;
      if (seen > row.lastSeenMs) row.lastSeenMs = seen;
      row.banned = banActive(r.banned_until, now);
    }
  } catch (e) {}

  // 補角色名：先 char_names，再 cloud_slots（真正在線角色）
  const needChars = [];
  for (const row of byAcc.values()) {
    if (!row.chars.length) needChars.push(row.account);
  }
  if (needChars.length) {
    try {
      for (let i = 0; i < needChars.length && i < 80; i++) {
        const acc = needChars[i];
        const charRows = await sql`
          SELECT name, account, slot FROM char_names
          WHERE account = ${acc} OR lower(account) = ${String(acc).toLowerCase()}
          ORDER BY slot LIMIT 8`;
        for (const ch of charRows) {
          const row = ensure(ch.account || acc);
          if (!row) continue;
          if (row.chars.some((c) => c.name === ch.name && Number(c.slot) === Number(ch.slot))) continue;
          row.chars.push({
            slot: Number(ch.slot) || 0,
            name: ch.name,
            lv: 0,
            clsName: "",
            mapId: "",
  ...
  const stillNeed = [];
  for (const row of byAcc.values()) {
    if (!row.chars.length) stillNeed.push(row.account);
  }
  if (stillNeed.length) {
    try {
      for (let i = 0; i < stillNeed.length && i < 80; i++) {
        const acc = stillNeed[i];
        const ak = accountKey(normalizeAccountId(acc) || acc);
        if (!ak) continue;
        const slotRows = await sql`
          SELECT slot,
            ...
            data->'p'->>'cls' AS cls,
            data->'p'->>'lv' AS lv
          FROM cloud_slots WHERE account_key = ${ak} ORDER BY slot LIMIT 8`;
        const row = ensure(acc);
        if (!row) continue;
        for (const ch of slotRows) {
          if (!ch.name) continue;
          if (row.chars.some((c) => c.name === ch.name && Number(c.slot) === Number(ch.slot))) continue;
          row.chars.push({
            slot: Number(ch.slot) || 0,
  ...
  list = list.slice(0, 100).map((row) => ({
    account: row.account,
    source: "online",
    online: true,
    banned: !!row.banned,
    lastSeenMs: row.lastSeenMs || 0,
    charCount: (row.chars || []).length,
    chars: row.chars || [],
    maps: row.maps || [],
    mapLabel: (row.maps || []).slice(0, 2).join("、") || "",
  }));
  ...
  return jsonRes(res, 200, {
    ...
    online: list,
    total: list.length,
    ttlMs: Math.max(PARTY_TTL_MS, ACCOUNT_SESSION_TTL_MS),
  });
}
...
async function gmSlotInventory(res, sql, raw, slotRaw) {
  ...
  const rows = await sql`
    SELECT data, updated_at FROM cloud_slots
    WHERE account_key = ${acc.account_key} AND slot = ${slot} LIMIT 1`;
  ...
  if (!data || !data.p) return jsonRes(res, 404, { ok: false, error: "no_save", message: "雲端存檔格式異常。" });
  const p = data.p;
  const db = loadItemDb() || {};
  const CLS = {
    royal: "王族",
  ...
  function itemLabel(it) {
    if (!it || !it.id) return "（空）";
    const id = String(it.id || "");
    const d = db[id];
    const n = (d && d.n) || id;
    const bits = [n];
    if (it.en) bits.push("+" + it.en);
    if (it.bless === true || it.bless === "blessed") bits.push("祝福");
    if (it.bless === "cursed") bits.push("詛咒");
    if (it.anc) bits.push("古代");
    if (it.cnt > 1) bits.push("×" + it.cnt);
    return bits.join(" ");
  }

  const invRaw = Array.isArray(p.inv) ? p.inv : [];
  const eqMap = p.eq && typeof p.eq === "object" ? p.eq : {};
  const inv = invRaw.slice(0, 500).map((it, idx) => ({
    idx,
    id: String((it && it.id) || ""),
    name: itemLabel(it),
  ...
  const eq = [];
  for (const [key, label] of EQ_SLOTS) {
    const it = eqMap[key];
    eq.push({
      slot: key,
      label,
      empty: !(it && it.id),
      name: it && it.id ? itemLabel(it) : "—",
  ...
  return jsonRes(res, 200, {
    ...
    summary: {
      ...
      name: p.name || "（未命名）",
      cls: p.cls || "",
      clsName: CLS[p.cls] || p.cls || "",
      lv: Number(p.lv) || 1,
      gold: Math.max(0, Math.floor(Number(p.gold) || 0)),
      invSlots: invRaw.length,
      invItems: inv.reduce((s, it) => s + (it.cnt || 1), 0),
      eqCount: eq.filter((r) => !r.empty).length,
      savedAt: Number(p.savedAt || data.savedAt || 0) || 0,
    },
    gold: Math.max(0, Math.floor(Number(p.gold) || 0)),
    // 相容舊 admin-gm.html：純 inv 精簡欄位 + 桌面後台完整欄位
    inv,
    eq,
  });
}
...
async function handleGmApi(req, res, u) {
  ...
  if (req.method === "GET") {
    ...
    if (u === "/api/gm/online") return gmOnlinePlayers(res, sql, q.get("q"));
