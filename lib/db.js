...
async function ensureSchema() {
  ...
  _schemaReady = (async () => {
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_account_sessions_last_seen ON account_sessions (last_seen_ms)`;
    ...
    await sql`CREATE TABLE IF NOT EXISTS rt_pandora_lot (
      id TEXT PRIMARY KEY,
      payload JSONB NOT NULL,
      updated_at BIGINT NOT NULL
    )`;
    await sql`CREATE TABLE IF NOT EXISTS rt_pandora_buy_orders (
      account TEXT NOT NULL,
    ...
    await sql`CREATE TABLE IF NOT EXISTS rt_pandora_claims (
      account TEXT NOT NULL,
      claim_id TEXT NOT NULL,
      payload JSONB NOT NULL,
      at BIGINT NOT NULL,
    ...
    await sql`CREATE TABLE IF NOT EXISTS rt_map_mob_rooms (
      map_id TEXT PRIMARY KEY,
      payload JSONB NOT NULL,
      updated_at BIGINT NOT NULL
    )`;
    await sql`CREATE INDEX IF NOT EXISTS idx_rt_map_mob_rooms_updated ON rt_map_mob_rooms (updated_at)`;
    await sql`ALTER TABLE accounts ADD COLUMN IF NOT EXISTS banned_until BIGINT NOT NULL DEFAULT 0`;
    await sql`ALTER TABLE accounts ADD COLUMN IF NOT EXISTS ban_reason TEXT NOT NULL DEFAULT ''`;
    await sql`CREATE TABLE IF NOT EXISTS server_config (
      key TEXT PRIMARY KEY,
      value JSONB NOT NULL,
      updated_at BIGINT NOT NULL
    )`;
    await sql`CREATE TABLE IF NOT EXISTS gm_grants (
      ...
      en INT NOT NULL DEFAULT 0,
      bless BOOLEAN NOT NULL DEFAULT FALSE,
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_gm_grants_pending ON gm_grants (account_key) WHERE claimed_at IS NULL AND cancelled_at IS NULL`;
    await sql`CREATE TABLE IF NOT EXISTS gm_audit (
      id BIGSERIAL PRIMARY KEY,
      at BIGINT NOT NULL,
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_gm_audit_at ON gm_audit (at DESC)`;
  })();
  return _schemaReady;
}
