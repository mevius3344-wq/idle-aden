...
const WB_RESPAWN_MS = Math.max(30000, Number(process.env.WB_RESPAWN_MS || 300000));
...
async function handleWorldBossApi(req, res, u, jsonFn) {
  ...
  if (u === "/api/worldboss/status" && req.method === "GET") {
    ...
    if (!mapId) {
      ...
      return send(res, 200, { ok: true, bosses: all });
    }
    return send(res, 200, { ok: true, state: wbPublicState(wbGetState(mapId)) });
  }
