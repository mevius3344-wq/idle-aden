...
async function handleAccountsApi(req, res, u) {
  ...
  if (u === "/api/accounts/login" && req.method === "POST") {
    ...
    return jsonRes(res, 200, { ok: true, account: rows[0].account || account });
  }
...
async function collectLeaderboardEntries(sql) {
  ...
  for (const r of rows) {
    ...
    const entry = {
      ...
      prideFloor: Math.max(0, Math.floor(Number(p.prideFloor) || 0)),
      prideMs: Math.max(0, Math.floor(Number(p.prideMs) || 0)),
      riftMs: Math.max(0, Math.floor(Number(p.riftMs) || 0)),
    };
    const old = byId[id];
