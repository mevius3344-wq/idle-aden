...
async function collectLeaderboardEntries(sql) {
  ...
  for (const r of rows) {
    ...
    if (!old) byId[id] = entry;
      ...
      else if (entry.gold !== old.gold) keep = entry.gold > old.gold ? entry : old;
      ...
      if ((!keep.eq || !Object.keys(keep.eq || {}).length) && entry.eq && Object.keys(entry.eq).length) {
        keep = Object.assign({}, keep, { eq: entry.eq, accountKey: entry.accountKey || keep.accountKey, slot: entry.slot || keep.slot });
