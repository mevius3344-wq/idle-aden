...
const { handleFieldPvpApi } = require("./rt-field-pvp");
const _antiCheat = require("./rt-anti-cheat");
const { mergeWalletAuthority } = require("./rt-cloud-wallet");
const { handleWalletShopApi } = require("./rt-wallet-shop-api");
const { econAuthEnabled } = require("./rt-shop");
const { isWarehouseSharedName } = require("./rt-warehouse");
const {
...
const { countOnlinePlayersSql } = require("./rt-server-status");
const { handleGmApi, getEffectiveRatesSql, accountBanSql, heartbeatGmInfoSql, banMessage } = require("./rt-gm");
const {
  metricsAuthOk,
  metricsAuthRequired,
  ...
  processMemory,
  capacityHints,
} = require("./rt-server-metrics");

const ROOT = path.join(__dirname, "..");

// 封測預設每 IP 連線上限 1；可用 IP_SESSION_MAX 覆寫（不對外公告）
...
function readGameVersion() {
  try {
    const dataJs = require("fs").readFileSync(path.join(ROOT, "js", "00-data.js"), "utf8");
...
async function handleServerMetricsApi(req, res) {
  if (req.method === "OPTIONS") return corsPreflight(res, "GET,OPTIONS", "Content-Type, Authorization");
  if (req.method !== "GET") return jsonRes(res, 405, { ok: false, error: "method" });
  if (!String(process.env.METRICS_TOKEN || "").trim()) return metricsNotConfigured(res, jsonRes);
  if (!metricsAuthOk(req)) return metricsAuthRequired(res, jsonRes);
  const sql = getSql();
  await ensureSchema();
  const t0 = Date.now();
  const payload = await collectDbMetrics(sql);
  payload.latency.totalMs = Date.now() - t0;
...
async function handleServerStatusApi(req, res) {
  ...
  const sql = getSql();
  await ensureSchema();
  const rates = await getEffectiveRatesSql(sql);
  const onlinePlayers = await countOnlinePlayersSql(sql);
...
async function handleAccountsApi(req, res, u) {
  ...
  if (u === "/api/accounts/register" && req.method === "POST") {
    ...
    if (password.length < 6) return jsonRes(res, 400, { ok: false, error: "password too short", message: "密碼至少 6 個字元。" });
    ...
    await sql`INSERT INTO accounts (account_key, account, password, created_at) VALUES (${key}, ${account}, ${_antiCheat.hashPassword(password)}, ${Date.now()})`;
  ...
  if (u === "/api/accounts/login" && req.method === "POST") {
    ...
    if (!account) return jsonRes(res, 400, { ok: false, error: "bad account" });
    const key = accountKey(account);
    const rows = await sql`SELECT account, password, banned_until, ban_reason FROM accounts WHERE account_key = ${key} LIMIT 1`;
    if (!rows.length) {
      return jsonRes(res, 404, { ok: false, error: "missing", message: "帳號不存在，請先註冊。" });
    }
    if (!_antiCheat.verifyPassword(password, rows[0].password)) {
      return jsonRes(res, 401, { ok: false, error: "bad password", message: "帳號或密碼錯誤。" });
    }
    const bannedUntil = Number(rows[0].banned_until) || 0;
    if (bannedUntil > Date.now()) {
      return jsonRes(res, 403, {
        ok: false,
        error: "banned",
        bannedUntil,
        message: banMessage(bannedUntil, rows[0].ban_reason),
      });
    }
    if (_antiCheat.needsPasswordUpgrade(rows[0].password)) {
      const newHash = _antiCheat.hashPassword(password);
      await sql`UPDATE accounts SET password = ${newHash} WHERE account_key = ${key}`;
    }
  ...
  if (u === "/api/accounts/session/heartbeat" && req.method === "POST") {
    ...
    const gm = await heartbeatGmInfoSql(sql, key, data.slot);
    if (gm.ban) {
      await releaseAccountSessionSql(sql, key, auth.sessionId);
      return jsonRes(res, 403, gm.ban);
    }
    ...
    const rates = await getEffectiveRatesSql(sql);
    return jsonRes(res, 200, { ok: true, account: auth.account, gmMail: gm.mail, rates });
  }
...
async function handleCloudApi(req, res, u) {
  ...
  if (m) {
    ...
    if (req.method === "PUT") {
      ...
      // 客戶端 _cloudPutBody 包成 { save, account, authToken }
      ...
      // B：Neon 雲端寫入必須帶登入令牌＋存檔合理性檢查（與 Render _serve.js 對齊）
      if (account !== CLOUD_ACCOUNT_DEFAULT && account !== "guest") {
        const auth = _antiCheat.authFromBody(
          { account: (parsed && parsed.account) || account, authToken: parsed && parsed.authToken },
          ROOT
        );
        if (!auth.ok) {
          return jsonRes(res, 401, {
        ...
        if (accountKey(auth.account) !== ak) {
          return jsonRes(res, 403, { ok: false, error: "account_mismatch", message: "帳號與登入令牌不符。" });
        }
        ...
        const v = _antiCheat.validatePlayerSave(data.p);
        if (v && !v.ok && v.severe) {
      ...
      if (existingRows.length) {
        ...
        // B2：拍賣等伺服器扣款序號較新時，金幣以伺服器為準，避免本機舊金幣覆蓋
        if (existing && existing.p && data.p) mergeWalletAuthority(data, existing);
  ...
  if (m) {
    ...
    if (req.method === "PUT") {
      // 🌐 P5c：經濟權威開啟時禁止自由覆寫倉庫（須走 /api/warehouse/move）
      if (econAuthEnabled() && isWarehouseSharedName(name)) {
        return jsonRes(res, 403, {
          ok: false,
          error: "warehouse_auth",
          message: "倉庫請使用存／領 API（/api/warehouse/move），不可直接覆寫。",
        });
      }
      let data;
      try {
...
async function collectLeaderboardEntries(sql) {
  ...
  for (const r of rows) {
    ...
    if (!display) continue; // 未命名不上榜
    const id = "name:" + charNameKey(display);
    ...
    const entry = {
      ...
      walletRev: Math.max(0, Math.floor(Number(p._walletRev) || 0)),
    ...
    let keep = old;
    if (entry.lv !== old.lv) keep = entry.lv > old.lv ? entry : old;
    else if (entry.exp !== old.exp) keep = entry.exp > old.exp ? entry : old;
    else if (entry.prideFloor !== old.prideFloor) keep = entry.prideFloor > old.prideFloor ? entry : old;
    else if (entry.prideFloor > 0 && old.prideFloor > 0 && entry.prideMs !== old.prideMs) {
      keep = entry.prideMs < old.prideMs ? entry : old;
    } else if (entry.riftMs !== old.riftMs) keep = entry.riftMs > old.riftMs ? entry : old;
    else keep = entry.walletRev >= old.walletRev ? entry : old;

    // 金幣：優先採較新 walletRev 的快照（經濟權威寫入後序號較高）
    const goldSrc =
      entry.walletRev !== old.walletRev
        ? entry.walletRev > old.walletRev
          ? entry
          : old
        : entry.gold >= old.gold
          ? entry
          : old;
    if (keep.gold !== goldSrc.gold || keep.walletRev !== goldSrc.walletRev) {
      keep = Object.assign({}, keep, {
        gold: goldSrc.gold,
        walletRev: goldSrc.walletRev,
        accountKey: goldSrc.accountKey || keep.accountKey,
        slot: goldSrc.slot || keep.slot,
      });
    }

    // 保留較完整的裝備快照（避免勝出存檔缺 eq）
    if ((!keep.eq || !Object.keys(keep.eq || {}).length) && entry.eq && Object.keys(entry.eq).length) {
      keep = Object.assign({}, keep, {
        eq: entry.eq,
        accountKey: entry.accountKey || keep.accountKey,
        slot: entry.slot || keep.slot,
      });
    } else if (keep === entry && old.eq && Object.keys(old.eq).length && (!entry.eq || !Object.keys(entry.eq).length)) {
      keep = Object.assign({}, keep, { eq: old.eq });
    }
    byId[id] = keep;
  }
...
/** 傲慢／裂痕無成績者不佔 Top 名額 */
function leaderboardEligible(board, row) {
  if (!row || !row.name) return false;
  if (board === "pride") return (row.prideFloor || 0) > 0;
  if (board === "rift") return (row.riftMs || 0) > 0;
  return true;
}

function leaderboardSort(board, entries) {
  const list = entries.filter((row) => leaderboardEligible(board, row));
  if (board === "gold") list.sort((a, b) => b.gold - a.gold || b.lv - a.lv || a.name.localeCompare(b.name, "zh"));
  else if (board === "pride") {
    list.sort(
      (a, b) =>
        b.prideFloor - a.prideFloor || a.prideMs - b.prideMs || b.lv - a.lv || a.name.localeCompare(b.name, "zh")
    );
  } else if (board === "rift") {
    list.sort((a, b) => (b.riftMs || 0) - (a.riftMs || 0) || b.lv - a.lv || a.name.localeCompare(b.name, "zh"));
  } else list.sort((a, b) => b.lv - a.lv || b.exp - a.exp || a.name.localeCompare(b.name, "zh"));
  return list;
}

function leaderboardRowPayload(board, row, rank) {
  return {
    rank: rank,
    name: row.name,
    cls: row.cls,
...
async function leaderboardRefreshEquipFromDb(sql, row) {
  if (!row || !row.accountKey || !row.slot) return row;
  try {
    const rows =
      await sql`SELECT data FROM cloud_slots WHERE account_key = ${row.accountKey} AND slot = ${row.slot} LIMIT 1`;
    const p = rows[0] && rows[0].data && rows[0].data.p;
    if (p && p.eq) row.eq = p.eq;
  } catch (e) {}
  return row;
}
...
async function handleLeaderboardEquipApi(req, res, u, sql) {
  ...
  let data = leaderboardLoadPlayerEquip(leaderboardCache.entries, name);
  if (!data) {
  ...
  if (!data.eq || !Object.keys(data.eq).length) {
    const refreshed = await leaderboardRefreshEquipFromDb(sql, {
      accountKey: data.accountKey,
      slot: data.slot,
      eq: null,
    });
    if (refreshed && refreshed.eq) data.eq = leaderboardSanitizeEquip(refreshed.eq);
  }
  return jsonRes(res, 200, {
    ok: true,
    name: data.name,
...
async function handleLeaderboardApi(req, res, u) {
  ...
  await ensureSchema();
  const q = new URL(req.url || "/", "http://localhost");
  const view = String(q.searchParams.get("view") || "").toLowerCase();
  // 相容：/api/leaderboard/equip 與 /api/leaderboard?view=equip&name=（避免部分主機巢狀路徑被改寫）
  if (u.startsWith("/api/leaderboard/equip") || (u === "/api/leaderboard" && view === "equip")) {
  ...
  const meName = normalizeCharName(q.searchParams.get("me") || "");
  const now = Date.now();
  if (!leaderboardCache.entries.length || now - leaderboardCache.at > 60000) {
    leaderboardCache = { at: now, entries: await collectLeaderboardEntries(sql) };
  }
  const sorted = leaderboardSort(board, leaderboardCache.entries);
  const top = sorted.slice(0, limit);
  const rows = top.map((row, i) => leaderboardRowPayload(board, row, i + 1));
  let me = null;
  if (meName) {
    const meKey = charNameKey(meName);
    const idx = sorted.findIndex((r) => charNameKey(r.name) === meKey);
    if (idx >= 0) {
      me = Object.assign(leaderboardRowPayload(board, sorted[idx], idx + 1), {
        inTop: idx < limit,
      });
    }
  }
  return jsonRes(res, 200, {
    ...
    total: sorted.length,
    pool: leaderboardCache.entries.length,
    rows,
    me,
  });
}
...
async function routeRequest(req, res, pathname) {
  ...
  try {
    if (u === "/api/server/status") return handleServerStatusApi(req, res);
    if (u === "/api/server/metrics") return handleServerMetricsApi(req, res);
    ...
    if (u.startsWith("/api/party") || u.startsWith("/api/map")) return handlePartyApi(req, res, u);
    if (u.startsWith("/api/clan")) return handleClanApi(req, res, u);
    if (u.startsWith("/api/auction")) return handleAuctionApi(req, res, u);
    if (
      u.startsWith("/api/wallet") ||
      u.startsWith("/api/shop") ||
      u.startsWith("/api/econ") ||
      u.startsWith("/api/warehouse")
    ) {
      return handleWalletShopApi(req, res, u);
    }
    if (u.startsWith("/api/pandora")) return handlePandoraApi(req, res, u);
    if (u.startsWith("/api/pvp")) return handleFieldPvpApi(req, res, u);
    if (u.startsWith("/api/gm/")) return handleGmApi(req, res, u);
