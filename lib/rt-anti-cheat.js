...
function issueAuthToken(account, root, sessionId) {
  const acc = String(account || "").trim().slice(0, 32);
...
function authFromBody(body, root) {
  ...
  return { ok: true, account, sessionId: parsed.sessionId, error: null };
}
