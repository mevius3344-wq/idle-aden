 ...
 * B 路線：金幣／經驗超量視為嚴重（拒絕上傳），其餘可 clamp 後接受。
 */
function validatePlayerSave(p) {
  ...
  // 極端洗白：超過等級上限 3 倍 → 一律拒絕（無法 clamp 洗白）
  if (gold > maxGold * 3) issues.push("gold_extreme");
  const exp = Math.max(0, Math.floor(Number(p.exp) || 0));
  const maxExp = maxExpForLevel(lv);
  if (exp > maxExp) issues.push("exp");
  if (exp > maxExp * 3) issues.push("exp_extreme");
  ...
  const severe = issues.some((x) =>
    ["gold", "exp", "gold_extreme", "exp_extreme", "panacea", "inv_slots", "inv_cnt", "no_player"].includes(x)
  );
  const clamp = {
    lv,
