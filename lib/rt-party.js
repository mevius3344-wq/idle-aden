...
function partyRequireAuth(data, res) {
  if (!_antiCheat) return true;
...
function mapMobElectHost(presenceMap, mapId, now) {
...
async function mapMobStoreSync(sql, mapId, hostKey, pack) {
  const id = String(mapId || "").slice(0, 64);
  ...
  await sql`INSERT INTO rt_map_mob_rooms (map_id, payload, updated_at)
    VALUES (${id}, ${row}, ${now})
    ON CONFLICT (map_id) DO UPDATE SET payload = EXCLUDED.payload, updated_at = EXCLUDED.updated_at`;
  return row;
}

async function mapMobSyncForViewer(sql, presenceMap, mapId, viewerKey, now) {
  const id = String(mapId || "").slice(0, 64);
  if (!mapMobMapAllowed(id) || !viewerKey) return null;
  const t = now || Date.now();
  const rows = await sql`SELECT payload, updated_at FROM rt_map_mob_rooms WHERE map_id = ${id} LIMIT 1`;
  if (!rows.length) return null;
  const row = rows[0].payload || {};
  const updatedAt = Number(rows[0].updated_at) || Number(row.at) || 0;
  if (t - updatedAt > MAP_MOB_SYNC_TTL_MS) {
    await sql`DELETE FROM rt_map_mob_rooms WHERE map_id = ${id}`;
    return null;
  }
  if (row.hostKey && row.hostKey === viewerKey) return null;
  const host = mapMobElectHost(presenceMap, id, t);
...
async function mapMobCleanup(sql, now) {
  const cutoff = (now || Date.now()) - MAP_MOB_SYNC_TTL_MS;
  await sql`DELETE FROM rt_map_mob_rooms WHERE updated_at < ${cutoff}`;
}

function partyMapPopulationPayload(presenceMap, mapId, now, excludeKey) {
  const t = now || Date.now();
  const counts = {};
  const players = [];
  const cur = String(mapId || "").slice(0, 64);
  for (const [key, pre] of presenceMap.entries()) {
    ...
    if (cur && id === cur && key !== excludeKey && players.length < 24) {
      players.push({
        key,
  ...
  players.sort((a, b) => String(a.key || "").localeCompare(String(b.key || "")));
  return {
    counts,
...
async function partyUpsertPresence(sql, parties, presenceMap, body) {
  ...
  const row = {
    ...
    // C：場戰世界座標（與 exploreCam 同單位），供同圖其他玩家顯示真實位置
...
async function handlePartyApi(req, res, u) {
  ...
  if (u === "/api/map/population" && req.method === "GET") {
    ...
    await prunePresence(sql, Date.now());
    await mapMobCleanup(sql, Date.now());
    const now = Date.now();
    const mapPop = partyMapPopulationPayload(presenceMap, mapId, now, selfKey);
    const mapHost = mapMobElectHost(presenceMap, mapId, now);
    const mapMobs = selfKey ? await mapMobSyncForViewer(sql, presenceMap, mapId, selfKey, now) : null;
  ...
  if (u === "/api/party/heartbeat" && req.method === "POST") {
    ...
    if (!partyRequireAuth(data, res)) return;
    const parties = await loadAllParties(sql);
    const presenceMap = await loadPresenceMap(sql);
    ...
    const mapId = up.presence && up.presence.mapId ? up.presence.mapId : "";
    const nowHb = Date.now();
    await mapMobCleanup(sql, nowHb);
    // refresh presence map entry for elect
    presenceMap.set(up.key, { ...(up.presence || {}), lastSeen: nowHb, key: up.key });
    const mapHost = mapMobElectHost(presenceMap, mapId, nowHb);
    if (data.mapMobs && mapHost && up.key === mapHost) {
      const mm = data.mapMobs;
      const mid = String(mm.mapId || mapId || "").slice(0, 64);
      if (mid && mid === mapId) await mapMobStoreSync(sql, mid, up.key, mm);
    }
    ...
    const mapMobs = await mapMobSyncForViewer(sql, presenceMap, mapId, up.key, nowHb);
    const partyMobs = mapMobs ? null : party ? partyMobSyncForMember(party, up.key, mapId) : null;
    const mapPop = partyMapPopulationPayload(presenceMap, mapId, nowHb, up.key);
    const seq = await getCounter(sql, "party_event_seq");
    return jsonRes(res, 200, {
      ok: true,
      key: up.key,
  ...
  if (u === "/api/party/create" && req.method === "POST") {
    ...
    const party = { id, leaderKey: up.key, members: [member], createdAt: Date.now(), lastShareAt: 0, shareAtByKey: {}, mobSync: null, applications: [] };
    parties.push(party);
    await saveParty(sql, party);
  ...
  if (u === "/api/party/share" && req.method === "POST") {
    ...
    // B2：收斂客戶端回報上限（對齊本機 _serve；真結算仍待擊殺權威）
