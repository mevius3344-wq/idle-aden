...
function _wcItemSourceAnswers(itemId) {
    ...
    if (d.gachaWeight > 0) return [
        `${d.n}目前在怪物專屬掉落表查不到直接來源，但有進潘朵拉抽抽樂池；也可能屬於製作、任務或兌換品。`,
        `查不到哪隻怪會直接掉${d.n}。先看製作與任務 NPC，潘朵拉抽抽樂也有機會出現。`
    ];
    return [
...
const WC_TOPICS = [
    ...
    },
    {
        ...
        gen: function () {
            return [
                ...
                '潘朵拉抽抽樂是付費權重抽獎：單抽固定金幣，十連九折；依物品權重隨機出貨，珍稀會世界廣播。',
                '想快賣裝備可去商店；村莊不再刷新叫賣玩家 NPC。'
            ];
        }
    },
    {
    ...
    },
    {
        ...
        gen: function () {
            return [
                '現行一般頭目掉落與製作裝備有 10% 機率出現「祝福的」詞綴；其他管道維持原設定。',
                '別再照舊攻略 SL 三詞綴了：祝福靠掉落／製作（頭目 10%）；屬性用四屬性卷軸、遠古用古代的卷軸，都去象牙塔找碧恩（各 7%）。',
    ...
    {
        key: 'merc', kw: ['傭兵', '協力', '隊友', '組隊', '幫手', '雇傭', '招募', '能招', '怎麼招', '傭兵公會', '卡住', '技能卡住', '屠宰者', '七個傭兵', '7個傭兵', '外觀', '變身能力'],
        gen: function () {
            return [
                '離線存檔位協力招募已從遊戲中移除。',
                '想組隊請用即時組隊；想帶戰力請用寵物、召喚術或城堡護衛。',
                '僅海音保留「創立血盟」NPC；其餘村莊請用右側血盟分頁創立或搜尋加入。',
    ...
    // ===== v3.6.61 依巴哈玩家實際問法補的六個主題 =====
    {
        ...
        gen: function () {
            return [
                ...
                '攻城區與村莊安全區死亡都不噴裝備；攻城區死亡也不扣經驗。',
                '死亡掉自身物品比照天堂：正義越高機率越低（最多 1 件）、中立有機率掉 1 件、邪惡越深機率越高且最多 1～3 件。',
                '噴掉的裝備別放棄：亞丁的聖使阿卡塔會記錄最近遺失的幾件，花龍之鑽石就能指定贖回其中一件，強化值跟詞綴都照原樣回來。',
    ...
    },
    {
        ...
        gen: function () {
            return [
                '殺怪會慢慢累積性向值；野外隨機玩家 NPC 遭遇已關閉，性向與復仇名單仍保留。',
                '紅名想洗白就乖乖打怪把性向養回來；殺紅名不扣性向。正義／中立／邪惡死亡都可能掉自身物品（比照天堂），越紅越危險，想清楚再走這條路。',
                '野外開 PK 擊殺真人：對方依性向真實失去物品，你撿到的就是那一批（強化詞綴照原樣），不會憑空多或少。',
                '正義值高補血法術會變強，究極光裂術也只有正義玩家能用；走正義路線不是只有名字好看。',
                '復仇名單與性向值仍保留；決鬥競技場（古魯丁巴魯特）可與其他玩家名片對戰。',
                '殺怪性向 +1；野外不再刷新玩家 NPC，性向相關規則仍適用於決鬥與攻城。',
                '世界頻道仍可嘲諷／私訊假玩家，但不會再在野外刷出追殺 NPC。',
    ...
    },
    {
        ...
        gen: function () {
            return [
                '潘朵拉抽抽樂是付費權重抽獎：單抽金幣、十連九折；權重池與舊黑市相同，珍稀會世界廣播。',
                '龍之鑽石主要靠村莊裡喊收購的玩家：把他要的東西給他就換龍鑽，物品放倉庫也算數。',
                '龍鑽存夠可以在黑市搜索指定類型的遺物；選「未知遺物」保證出圖鑑還沒收錄的，蛋類遺物只有這個管道。',
    ...
    },
    {
        ...
        gen: function () {
            return [
                ...
                '版本更新後約 45 秒內會自動偵測並重新載入；若畫面仍怪再按 Ctrl+Shift+R 硬重載清快取。',
...
function _wcPostIdleChat() {
    return;   // 假玩家閒聊／互嗆已關閉：世界頻道改為僅真實玩家發言
    if (!_wcCanIdleChat()) return;
...
function _chatOnlineSend(payload) {
    if (typeof window !== 'undefined' && typeof window.gameOnlineSuspended === 'function' && window.gameOnlineSuspended()) return Promise.resolve(false);
    if (typeof window !== 'undefined' && (window.__DEV_OFFLINE || window.__onlineIdleForced || window.__wildOnlineForced)) return Promise.resolve(false);
...
function _chatAppend(ch, html, cls) {
    let el = _chatLogEl(ch);
    if (!el) return;
    el.insertAdjacentHTML('beforeend', `<div class="log-entry ${cls || ''}">${html}</div>`);
    let locked = (typeof _worldLogLocked !== 'undefined') && _worldLogLocked && ch === _chatActiveChannel;
    let _max = locked ? (typeof WORLD_LOG_MAX_LOCKED !== 'undefined' ? WORLD_LOG_MAX_LOCKED : 400) : (typeof WORLD_LOG_MAX !== 'undefined' ? WORLD_LOG_MAX : 200);
    while (el.children.length > _max && el.children.length > 1) el.removeChild(el.firstChild);
    if (!(typeof _worldLogLocked !== 'undefined' && _worldLogLocked && ch === _chatActiveChannel)) el.scrollTop = el.scrollHeight;
}
...
function _chatPartyPeersOk(payload) {
    ...
    if (!!payload.classic !== classic) return false;
    return _roleOtherActiveSessions().some(s => s && (s.fp === payload.fp || String(s.slot) === String(payload.slot)));
}
function _chatCanReceive(payload) {
    if (!payload || !payload.ch || !payload.text) return false;
    if (typeof player === 'undefined' || !player || !player.cls) return false;
    if (!!payload.classic !== !!player.classicMode) return false;
    if (payload.ch === 'world') return true;
    if (payload.ch === 'clan') return !!_chatClanKey() && payload.clanKey === _chatClanKey();
    if (payload.ch === 'party') return _chatPartyPeersOk(payload);
    return false;
}
function _chatRenderLine(payload, isSelf) {
    let name = payload.name || '未知';
    let align = payload.alignment;
    let nameHtml = (typeof _wcStaticNameHtml === 'function')
        ? _wcStaticNameHtml(name, align)
        : `<b>${String(name).replace(/[<>&]/g, '')}</b>`;
    let text = (typeof _wcEsc === 'function') ? _wcEsc(payload.text) : String(payload.text).replace(/[<>&]/g, '');
    let tag = payload.ch === 'clan' ? '<span class="wc-ch-tag wc-ch-clan">［血盟］</span> '
        : payload.ch === 'party' ? '<span class="wc-ch-tag wc-ch-party">［隊伍］</span> ' : '';
...
function _chatDeliver(payload, fromRemote) {
    ...
    if (payload.id) {
        if (_chatSeenIds[payload.id]) return;
        _chatSeenIds[payload.id] = Date.now();
        let keys = Object.keys(_chatSeenIds);
...
function worldChannelAsk() {
    ...
    if (typeof player === 'undefined' || !player || !player.cls) {
        _chatAppend(_chatActiveChannel, '<span class="wc-sys">請先進入遊戲再發言。</span>');
        return;
    }
    let now = Date.now();
    if (now < _chatAskCooldownUntil) {
        _chatAppend(_chatActiveChannel, '<span class="wc-sys">發言過快，請稍候。</span>');
        return;
    }
    _chatAskCooldownUntil = now + 1200;
    input.value = '';

    let ch = _chatActiveChannel || 'world';
    if (ch === 'clan' && !_chatClanKey()) {
        _chatAppend('clan', '<span class="wc-sys">尚未加入血盟，無法使用血盟頻道。</span>');
        return;
    }
    if (ch === 'party') {
    ...
    let payload = {
        v: 1,
        id: 'c' + now.toString(36) + '_' + Math.random().toString(36).slice(2, 8),
        ch: ch,
        text: q.slice(0, 60),
        name: player.name || '未命名',
        alignment: player.alignmentValue || 0,
...
if (_chatBc) {
    _chatBc.onmessage = function (ev) {
        try { _chatDeliver(ev.data, true); } catch (e) {}
    };
}
window.addEventListener('storage', function (ev) {
    if (!ev || ev.key !== 'fb5_real_chat_ping_v1' || !ev.newValue) return;
    try {
        let wrap = JSON.parse(ev.newValue);
