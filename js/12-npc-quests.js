...
function renderIoExchange(div) {
    ...
    div.innerHTML = `<div class="flex flex-col items-center justify-center py-12 gap-3 text-slate-400">
        ...
        <span class="text-sm text-center">伊奧兌換與席琳遺骸相關內容已自遊戲中移除。</span>
    </div>`;
}
function doIoExchange() {
...
function renderLachesisSplit(div) {
    ...
    div.innerHTML = `<div class="flex flex-col items-center justify-center py-12 gap-3 text-slate-400">
        <span class="text-5xl">🚫</span>
        <span class="text-xl font-bold text-amber-200">席琳系統已關閉</span>
        <span class="text-sm text-center">菈克希絲拆分與席琳詞綴相關內容已自遊戲中移除。</span>
    </div>`;
}
function doLachesisSplit() {
    logSys('<span class="text-amber-300">席琳系統已移除。</span>');
}
