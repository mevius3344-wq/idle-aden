...
const SAVE_DEFAULTS = {
    ...
    magicShieldCd: 0, reviveScrollCd: 0, lastMapByCat: {}, lastBattleMap: null, tracking: null, sherineWorld: false, sherineMad: false, classicMode: true, traditionalMode: false,
...
// ============================================================================
function sherineWorldActive() { return false; }   // 席琳系統已移除
function sherineMadActive() { return false; }     // 席琳系統已移除
function applySherineTheme() {
    try {
        document.body.classList.remove('sherine-world', 'sherine-mad');
        if (typeof player !== 'undefined' && player) {
            player.sherineWorld = false;
            player.sherineMad = false;
        }
    } catch (e) {}
}
function sherineSystemRemoved() { return true; }
/** 清除席琳遺骸／結晶／套裝詞綴；強制關閉席琳世界 */
function purgeSherineContent() {
    try {
        if (typeof player === 'undefined' || !player) return;
        player.sherineWorld = false;
        player.sherineMad = false;
        player._sherineSetCnt = {};
        if (player.lastTownVisited === 'town_sherine') player.lastTownVisited = null;
        if (!player.eq) player.eq = {};
        ['rem_claw','rem_eye','rem_blood','rem_flesh','rem_heart','rem_bone','rem_fang','rem_scale'].forEach(k => { player.eq[k] = null; });
        const isSherineItem = (it) => {
            if (!it || !it.id) return false;
            if (it.id === 'sherine_crystal') return true;
            if (String(it.id).indexOf('rem_') === 0) return true;
            let d = (typeof DB !== 'undefined' && DB.items) ? DB.items[it.id] : null;
            return !!(d && (d.remains || d.n === '席琳結晶'));
        };
        const stripSeteff = (it) => { if (it && it.seteff) it.seteff = false; };
        if (Array.isArray(player.inv)) {
            player.inv = player.inv.filter(it => !isSherineItem(it));
            player.inv.forEach(stripSeteff);
        }
        Object.keys(player.eq).forEach(k => {
            if (String(k).indexOf('rem_') === 0) { player.eq[k] = null; return; }
            stripSeteff(player.eq[k]);
        });
        try {
            if (typeof loadWarehouse === 'function' && typeof saveWarehouse === 'function') {
                let wh = loadWarehouse();
                if (wh && Array.isArray(wh.items)) {
                    let before = wh.items.length, dirty = false;
                    wh.items = wh.items.filter(it => !isSherineItem(it));
                    if (wh.items.length !== before) dirty = true;
                    wh.items.forEach(it => { if (it && it.seteff) { it.seteff = false; dirty = true; } });
                    if (dirty) saveWarehouse(wh);
                }
            }
        } catch (e) {}
        applySherineTheme();
...
const MASTERY_DATA = {
    ...
    mage: { logo: 'assets/logo/法師logo.png', boss: '飛龍', list: {
        ...
        m_summon:    { n: '召喚精通', pos: 'bottom', msg: '強化迷魅與召喚眷屬', d: '迷魅成功率吃魅力；對等級不高於自己的非頭目幾乎必成。迷魅僕人繼承原怪傷害加成與命中，魅力再提高命中與傷害；造屍／召喚／迷魅眷屬同享傷害、命中與技能頻率強化。' }
    } },
    ...
    dark: { logo: 'assets/logo/黑暗妖精logo.png', boss: '飛龍', list: {
        d_poison: { n: '劇毒精通', pos: 'top',    msg: '使附加劇毒更致命',     d: '將劇毒深植兵刃，使每次淬毒攻擊都留下猛烈而持續的傷害。' },
...
function worldLogToBottom() {
    let el = (typeof _chatLogEl === 'function' && typeof _chatActiveChannel !== 'undefined')
        ? _chatLogEl(_chatActiveChannel)
        : document.getElementById('world-log');
    if (!el) return;
...
function initWorldLogLock() {
    ['world-log', 'clan-log', 'party-log'].forEach(function (id) {
        let el = document.getElementById(id);
        if (!el || el._lockInit) return;
        el._lockInit = true;
        let onScroll = () => {
            if (typeof _chatActiveChannel !== 'undefined') {
                let activeId = ({ world:'world-log', clan:'clan-log', party:'party-log' })[_chatActiveChannel] || 'world-log';
