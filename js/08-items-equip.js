...
function getItemColor(item) {
    ...
    // 🔮 席琳套裝效果：鮮綠＋呼吸綠光（系統已移除；殘留 seteff 不再上色）
    if (false && item.seteff) return 'c-sherine';
...
function getGlowClass(item, d) {
    // 🔮 席琳結晶／套裝圖示光（系統已移除）
    if (false && ((item && item.id === 'sherine_crystal') || (d && d.n === '席琳結晶'))) return 'sherine-glow-icon';
    if (false && item && item.seteff) return 'sherine-glow-icon';
...
function renderStatusEffects() {
    ...
    // 🎉 全服倍率活動（GM 後台設定）
    if (typeof serverExpEventMult === 'function') {
        let remain = (typeof serverExpEventRemainLabel === 'function') ? serverExpEventRemainLabel() : '';
        let _rs = remain ? '（' + remain + '）' : '';
        let _em = serverExpEventMult(), _gm = serverGoldEventMult(), _dm = serverDropEventMult();
        if (_em > 1) buffs.push(`<span class="text-yellow-300 font-bold" title="全服打怪經驗 ×${_em}">經驗×${_em}${_rs}</span>`);
        if (_gm > 1) buffs.push(`<span class="text-yellow-300 font-bold" title="全服金幣 ×${_gm}">金幣×${_gm}${_rs}</span>`);
        if (_dm > 1) buffs.push(`<span class="text-yellow-300 font-bold" title="全服掉寶 ×${_dm}">掉寶×${_dm}${_rs}</span>`);
    }
