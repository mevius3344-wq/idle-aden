...
function currentRoleIsMercenary() { return false; }   // 傭兵系統已移除
...
function alliesTick() {
    if (player && player.allies && player.allies.length) player.allies = [];   // 傭兵系統已移除
    return;
    if (!player.allies || !player.allies.length) return;
    player.allies.forEach(ally => {
        if (!ally) return;
...
function toggleAlly(slotN) {
    logSys('<span class="text-amber-300">傭兵系統已關閉，無法招募或解散協力傭兵。</span>');
...
function dismissAlly(slotN) {
    if (player) player.allies = [];
    logSys('<span class="text-amber-300">傭兵系統已關閉。</span>');
    return;
    slotN = String(slotN);
    let ally = (player.allies || []).find(a => a && String(a._slot) === slotN);
    if (!ally) {
