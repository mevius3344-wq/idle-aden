    ...
    if(nameInput){
        ...
        try {
            if(typeof GameAccountAuth !== 'undefined' && GameAccountAuth && typeof GameAccountAuth.currentAccount === 'function')
                hint = String(GameAccountAuth.currentAccount() || '').trim();
        } catch(e){}
        nameInput.value = '';
        nameInput.placeholder = hint ? ('請輸入角色名稱（例：' + hint + '）') : '請輸入角色名稱';
...
function saveGame() {
    ...
    player.allies = [];   // 傭兵系統已移除：寫檔不保留協力傭兵
    ...
    return true;
        ...
        if(!_saveFailureNotified && typeof logSys === 'function') {
            ...
            logSys('<span class="text-red-400 font-bold">⚠ 遊戲進度儲存失敗，本次進度未寫入。請重新整理後再試。</span>');
        }
        return false;
    }
}
