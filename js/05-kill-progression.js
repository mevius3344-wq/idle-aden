...
function killMob(idx) {
    ...
    if (typeof serverDropEventMult === 'function') _dropBase *= serverDropEventMult();   // 🌐 全服掉寶活動
...
function renderRiftEntrance(container) {
    ...
    box.innerHTML = `
        ...
        ${''}
        <div class="text-slate-500 text-xs">進入需消耗 <span class="text-amber-300">1 顆 龜裂之核</span>（目前持有 ${cores}）。停留越久排名越前、獎勵越好；裂痕內無法傳送，離開後須先領取上次獎勵才能再次進入。</div>`;
